...
def _setuptools_available() -> bool:
    """Check whether setuptools is importable in the current interpreter."""
    try:
...
@pytest.mark.skipif(
    not _setuptools_available(),
    reason="setuptools not installed in runtime env (build-only dependency)",
)
