...
class TestValidAliases:
    ...
    def test_metabolite_class_aliases_do_not_map_to_annotation_in_0_2(self, col, expected):
        sm = detect_schema([col])
        assert sm.canonical_to_original.get("annotation") is None
        assert sm.rejected_candidates["annotation"][0]["column"] == expected
...
class TestPilotSTSchema:
    ...
    def test_main_and_sub_class_are_rejected_as_annotation_evidence(self):
        sm = detect_schema(self.COLUMNS)
        assert sm.canonical_to_original.get("annotation") is None
        assert [item["column"] for item in sm.rejected_candidates["annotation"]] == [
            "Main class",
            "Sub class",
        ]
...
class TestValidex02AliasGovernance:
    def test_governed_alias_registry_exposes_rationale_and_risk(self):
        sm = detect_schema(["identity evidence", "nominal probability"])

        annotation = sm.candidates["annotation"][0]
        p_value = sm.candidates["p_value"][0]

        assert annotation.alias == "identity_evidence"
        assert annotation.rationale
        assert annotation.risk == "medium"
        assert p_value.match_rule == "exact"
    ...
    def test_annotation_contract_rejects_descriptive_or_model_columns(self, col):
        sm = detect_schema([col])

        assert "annotation" not in sm.canonical_to_original
        assert sm.rejected_candidates["annotation"][0]["column"] == col
    ...
    def test_annotation_contract_accepts_identification_evidence_columns(self, col):
        sm = detect_schema([col])

        assert sm.canonical_to_original["annotation"] == col

    def test_multiple_annotation_candidates_are_reported_without_forced_selection(self):
        sm = detect_schema(["MSI level", "library match", "spectral evidence"])

        assert sm.canonical_to_original["annotation"] is None
        assert sm.ambiguity_status["annotation"] == "multiple_valid_no_preference"
        assert [c.column for c in sm.candidates["annotation"]] == [
            "MSI level",
            "library match",
            "spectral evidence",
        ]

    def test_preferred_candidate_selection_is_reported_with_reason(self):
        sm = detect_schema(["p_value", "nominal probability", "adjusted_p"])

        assert sm.canonical_to_original["p_value"] == "p_value"
        assert sm.ambiguity_status["p_value"] == "multiple_valid_preferred"
        assert sm.selection_reasons["p_value"] == "canonical_header_preferred"
        assert [c.column for c in sm.candidates["p_value"]] == [
            "p_value",
            "nominal probability",
        ]
