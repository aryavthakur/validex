...
from .ingestion import IngestionMetadata, ResourceLimits, ingest_table_path
...
def _classify_probability_value(value: Any) -> tuple[NumericCellCategory, float | None]:
    ...
    # NaN represents a missing cell (e.g. pandas coerces None to NaN in
    # string-typed columns). Classify as MISSING, not NONFINITE.
...
def _fdr_ordering_summary(
    df: pd.DataFrame,
    p_col: str | None,
    fdr_col: str | None,
    min_valid_fraction: float = 0.8,
) -> dict[str, Any]:
    if not p_col or not fdr_col:
        return {
            "comparable_row_count": 0,
            "fdr_greater_or_equal_count": 0,
            "fraction_fdr_greater_or_equal": None,
            "warning": False,
        }
    p_num = pd.to_numeric(df[p_col], errors="coerce")
    fdr_num = pd.to_numeric(df[fdr_col], errors="coerce")
    ...
    fdr_ge_p = int((fdr_num[both_valid] >= p_num[both_valid]).sum())
    fraction = fdr_ge_p / comparable
    return {
        "comparable_row_count": comparable,
        "fdr_greater_or_equal_count": fdr_ge_p,
        "fraction_fdr_greater_or_equal": fraction,
        "warning": fraction < min_valid_fraction,
    }
...
def audit_dataframe(
    ...
    # ------------------------------------------------------------------
    fdr_ordering = _fdr_ordering_summary(df, p_col_raw, fdr_col_raw)
    fdr_consistency_warning = bool(fdr_ordering["warning"])
    if fdr_consistency_warning:
        confidence -= 15
        msg = (
    ...
    for canonical, ambig_cols in schema.ambiguities.items():
        chosen = schema.canonical_to_original.get(canonical)
        is_critical = canonical in _CRITICAL_FIELDS

        if is_critical or chosen is None:
        ...
        if chosen is None:
            msg = (
                ...
                "Validex did not select one because there is no defensible deterministic preference. "
        ...
        flags.append(
            {
                ...
                "ambiguity_status": schema.ambiguity_status.get(canonical),
                "selection_reason": schema.selection_reasons.get(canonical),
                "why": msg,
    ...
    result = {
        ...
        "fdr_ordering": fdr_ordering,
        "schema_candidates": {
            field: [candidate.as_dict() for candidate in candidates]
            for field, candidates in schema.candidates.items()
        },
        "ambiguity": {
            field: {
                "status": schema.ambiguity_status.get(field, "no_valid_candidate"),
                "selected_column": schema.canonical_to_original.get(field),
                "candidate_columns": [
                    candidate.column for candidate in schema.candidates.get(field, [])
                ],
                "selection_reason": schema.selection_reasons.get(field),
            }
            for field in [
                "compound_id",
                "effect_size",
        ...
        "rejected_schema_candidates": schema.rejected_candidates,
...
def run_audit(
    ...
    ingested = ingest_table_path(csv_path, limits=limits)
