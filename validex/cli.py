...
from .ingestion import IngestionError, ResourceLimits, ingest_table_path
...
def command_audit(args: argparse.Namespace) -> int:
    """Run a schema audit on a supported table file and print results to the terminal."""
    table_path: str = args.input
    ...
    if not os.path.exists(table_path):
        print(
            f"Error [FILE_NOT_FOUND]: file not found: {os.path.basename(table_path)}",
            file=sys.stderr,
        )
        return 1

    try:
        ingested = ingest_table_path(
            table_path, limits=limits, sheet_name=getattr(args, "sheet", None)
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    audit = subparsers.add_parser("audit", help="Audit a metabolomics result table.")
    audit.add_argument("input", help="Path to the CSV, TSV, or XLSX file to audit.")
    audit.add_argument(
        "--sheet",
        default=None,
        help="XLSX sheet name to audit when a workbook contains multiple tables.",
    )
    audit.add_argument(
        "--output",
        "-o",
